...
describe Sprinkle::Installers::PushText do
  ...
  describe 'during installation' do
    ...
    it 'should invoke the push text installer for all specified packages' do
      @install_commands.should == %q[sh -c '/bin/echo -e '\\''another-hair-brained-idea'\\'' | tee -a /dev/mind/late-night']
    end

    it 'should automatically insert pre/post commands for the specified package' do
      @installer.send(:install_sequence).should == [ 'op1', "sh -c '/bin/echo -e '\\''another-hair-brained-idea'\\'' | tee -a /dev/mind/late-night'", 'op2' ]
    end
  ...
  describe 'running with sudo' do
    ...
    it "should invoke the push installer with sudo" do
      @install_commands.should == %q[sh -c '/bin/echo -e '\\''a special user'\\'' | tee -a /dev/mind/the-day-after']
    end
  end
  ...
  describe 'sending a string with single quotes' do
    ...
    it "should correctly encode the single quote character" do
      @install_commands.should == %q[sh -c '/bin/echo -e '\\''I'\\''\\'\\'''\\''m a string with a single quote'\\'' | tee -a /dev/mind/the-day-after']
    end
  end
